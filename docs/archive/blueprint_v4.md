...
### Politica di Prompting: Mediterranean Prompting (English Markers)
Per garantire la massima fedeltà linguistica e prevenire derive di accento inglese nel TTS, tutti i prompt della pipeline (Stage A, B, C, D) sono rigorosamente in **Italiano**. 
Tuttavia, lo Stage C (SceneDirector) utilizza **Official English Markers** (es: `(serious)`, `(sincere)`, `(scared)`, `(sad)`) e **Punteggiatura Drammatica** (es: `\n\n` per i titoli o `...` per le pause) per guidare il modello Fish S1-mini. Non vengono usati tag discorsivi o parentesi quadre (`[Instruction]`) poiché non supportati e deleteri per la qualità audio.

### Testing Offline (Privacy & Cost Optimization)
DIAS supporta una modalità di testing "Offline-First". Poiché ogni stadio salva l'output in formato JSON in `data/stage_X/output`, è possibile rieseguire test sulla pipeline o sull'integrazione ARIA utilizzando file pre-elaborati, senza chiamare nuovamente le API di Google Gemini.
...
# DIAS Pipeline v4.0 - Specifiche Funzionali Complete  
## Sistema di Produzione Audiobook Cinematico Distribuito

**Versione**: 4.0 (Fish S1-mini Integration)    
**Data**: Marzo 2026    
**Target Hardware**: RTX 5000 / RTX 4090 (Windows 11 Node) + Brain (LXC)    
**Stack API**: Google GenAI SDK (Gemini Flash-Lite Latest)    
**Stack Locale**: Fish Audio S1-mini (Distributed via ARIA)    
...
\`dias:queue:4:voice\_gen\` | D: VoiceGenerator (ARIA Proxy) | GPU Worker (Windows)  
...
dias:lock:gpu:fish\_s1\_mini                  \# "1" TTL 24h (fino a completamento)  
...
\*\*Output\*\*: Audio Script ottimizzato con marker Fish e pause drammatiche    
...
### COMPONENTE D: VoiceGenerator (GPU Windows via ARIA)  
\*\*Modello\*\*: \`Fish Audio S1-mini\`    
\*\*Ambiente\*\*: GPU Worker (Windows 11) via ARIA Proxy    
\*\*Input\*\*: Messaggio dalla coda 4    
\*\*Output\*\*: WAV 44.1kHz mono per scena (HTTP URL)    
\*\*Queue\*\*: Consume \`dias:queue:4:voice\_gen\` → Produce callback Redis

**Gestione VRAM e Context Limit**:  
Il VRAMManager di ARIA gestisce il caricamento del modello Fish S1-mini (~3GB VRAM). Per evitare il troncamento dell'audio (limite di ~1023 token), l'Orchestrator implementa un **Text Chunking automatico (max 150 parole)** dividendo il testo per frasi sui segni di punteggiatura forte, per poi rimettere insieme i file WAV in modo trasparente per DIAS.

**Coerenza Tra Scene**:  
- Utilizzare voice cloning con campione narratore (`C:\Users\Roberto\aria\data\voices\narratore.wav`)
- Parametri: `temperature: 0.7`, `top_p: 0.7` per stabilità massima.
- Marcatori emotivi estesi ufficiali (es. `(serious)`, `(scared)`, `(angry)`) e pause `\n\n` inseriti nello Stage C.
- **Accento Italiano**: Il campo `voice_ref_text` deve corrispondere esattamente al testo del file `narratore.wav`.

**Output**:  
- File: `C:\Users\Roberto\aria\data\outputs\{job_id}_{scene_id}.wav`  
- Aggiornamento Redis (Callback): URL HTTP accessibile dal LXC.
...
3\. Carica MusicGen (unload Fish se necessario)  
...
\#\#\# Step 4: VoiceGeneration (ARIA Windows)  
\- \*\*Request via ARIA\*\* (t=0)  
\- Scene 0: Generata in 12s (durata: 142s)  
\- Scene 1: Generata in 18s (durata: 198s)  
\- Scene 2: Generata in 15s (durata: 152s)  
\- \*\*Output\*\*: URL Asset HTTP registrati in Redis
...
models:  
  qwen3\_tts:  
    path: "fishaudio/openaudio-s1-mini"  
    device: "cuda"  
    max\_memory\_gb: 8  
    batch\_size: 1  
