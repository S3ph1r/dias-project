...
Il progetto utilizza un ambiente virtuale Python 3.10+ gestito tramite **[requirements.txt](../requirements.txt)**.

### **Core Libraries & Tools**
- **AI & LLM**: `google-genai` (Stage 0/B/C), `pydantic` (Data Models).
- **Audio Analysis**: `librosa`, `scipy`, `matplotlib` (Benchmarking e QC).
- **Audio Processing**: `pydub`, `ffmpeg` (Mixing e Mastering).
- **Persistence**: `redis` (Bus di stato), `json` (LocalStorage).
- **Web App**: `fastapi`, `uvicorn` (API Hub), `sveltekit` (Dashboard).
...
#### **`persistence.py`** - Project-Centric Storage Manager
- **Funzionalità**:
  - Instradamento automatico dei file nella nuova struttura `data/projects/{project_id}/`.
  - Normalizzazione ID (Slugify) per coerenza tra OS (Linux case-sensitivity handled via API Hub).
  - Gestione directory source, stages, logs e final per ogni progetto.
- **Metodi chiave**:
  ```python
  normalize_id(text) -> str  # Genera slug (es. cronache-del-silicio)
  save_stage_output(...)     # Salva JSON in data/projects/{id}/stages/...
  ```
...
#### **`stage_0_intel.py`** - Stage 0: Book Intelligence (NEW 2026)
- **🎯 Scopo**: Estrarre il "DNA" del libro (Chapter map, Casting, Sound Design).
- **Input**: Testo completo del libro (`.txt`) in `source/`.
- **Strategia**: Supporta la **Sequential Contextual Injection** per libri di grandi dimensioni.
- **Dettagli**: Vedi **[Guida Pre-produzione](./preproduction-guide.md)** per la specifica completa della strategia e il case study di Hyperion.
- **Integrazione Dashboard**: Alimenta la tab "Pre-production" permettendo il casting manuale.

#### **`stage_a_text_ingester.py`** - Stage A: Ingestione Documenti
- **🎯 Scopo**: Convertire PDF → Chunk testuali intelligenti (Project-Aware)
- **Input**: PDF in `data/projects/{id}/source/`
- **Output**: Chunk testuali e task su Redis.
- **Algoritmo chunking**:
  ```
