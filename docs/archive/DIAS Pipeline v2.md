...
**Queue**: Consume `dias:queue:3:scene_director` → Produce `dias:queue:4:voice_gen` (N messaggi, uno per scena)

**Logica Segmentazione (Emotional Beats & Qwen3 Optimized)**:  
Il modello non divide più il testo meccanicamente (es. conteggio parole), ma delega la segmentazione all'LLM basandosi sui "Batti Emotivi" (Emotional Beats).
- **Segmentazione Dinamica**: Una nuova scena (sub-chunk) viene creata ogni volta che avviene un cambio netto di tono, emozione, ritmo o atmosfera (es. shift da narrazione riflessiva a dialogo teso). Le scene hanno lunghezza variabile in base al respiro narrativo.
- **Normalizzazione Testuale**: Il testo viene "pulito", aggiungendo accenti fonetici sulle parole ambigue (es. *àncora* vs *ancòra*) e **traducendo tutti i numeri arabi in lettere** (es. "2042" → "duemilaquarantadue") per garantire una pronuncia impeccabile dal TTS.
- **Istruzione Stile (Instruct)**: Per ogni blocco emotivo delimitato, genera un campo `qwen3_instruct` in inglese (es. *"Warm Italian male voice, tense and restrained, slow deliberate pace"*) specifico per guidare Qwen3 in quella precisa sfumatura.
...
\### 6.2 Idempotenza e "Stage Skipping" (Self-Healing)
Per garantire che il sistema non sprechi tempo o risorse preziose (API, GPU), ogni stadio implementa la logica di **Skip Intelligente**:
1. Prima di processare un'unità (Chapter/Scene), lo stage interroga Redis e il filesystem locale.
2. Se il file di output (`.json` o `.wav`) per quell'ID esiste già ed è valido, lo stage produce immediatamente il messaggio di successo per la coda successiva senza rieseguire il calcolo.
3. Questo permette di riavviare l'intera pipeline dopo un crash e vederla "scivolare" via velocissima sui pezzi già pronti fino al punto di interruzione.

## 7. DIAS Dashboard: La Centrale di Controllo
L'integrazione finale prevede un'interfaccia web (LXC 190) che funge da supervisore dell'intero ecosistema.

### 7.1 Funzionalità Core
- **Job Manager**: Upload PDF, selezione voci (Luca/Angelo/User), impostazione parametri globali (Emozione base, Pacing).
- **Real-Time Monitoring**: Visualizzazione grafica del progresso (ProgressBar per capitolo, stato delle code Redis).
- **Node Status**: Monitoraggio dello stato di ARIA (GREEN/RED) e del carico GPU sul PC Gaming.
- **Recovery Tools**: Pulsante "Resume" per riaccodare task bloccati o orfani dopo un crash sistemico.
- **Preview & Master**: Player integrato per ascoltare le scene generate e pulsante "Generate Master" per il mix finale.

### 7.2 Architettura Dashboard
- **Backend**: FastAPI (Python) su LXC 190.
- **Frontend**: Streamlit o React per una UI moderna e reattiva.
- **Storage**: Directory centralizzata `/home/Projects/NH-Mini/sviluppi/dias/data/` accessibile a tutti gli stage.
...
5. **Dashboard First**: Progetta gli stage affinché scrivano il loro stato in Redis in modo che la Dashboard possa leggerlo senza polling eccessivo.
